...
def get_voicevox_download_link(version) -> str:
    ...
    assets = target_release["assets"]
    for asset in assets:
        if asset["name"] == "voicevox_core-linux-x64-gpu-0.13.1.zip":
...
def download_and_extract_voicevox(download_link):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        if os_name == "Windows":
            ...
            run_subprocess(
                f'powershell -Command "cd {tmp_dir}; Expand-Archive -Path voicevox_core-linux-x64-gpu-0.13.1.zip -Destination ./; Copy-Item core -Recurse {project_root}/release"')

        else:
            run_subprocess(
                f"cd {tmp_dir} && wget {download_link} && unzip voicevox_core-linux-x64-gpu-0.13.1.zip && cp -r voicevox_core-linux-x64-gpu-0.13.1 {project_root}/release"
            )
